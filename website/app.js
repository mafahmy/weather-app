...
let apiKey = "&appid=23c201ff8b3d5bb7708b00f61531dbcd";
...
function performAction(){
    ...
    .then(function(data){
        console.log(data);
        postData('/add',{temperature : data.main.temp, date : newDate, user_response: feels})      // calling the postData function 
    })
    .then (function() {
        updateUi()
    })
...
const postData = async ( url = '', projectData = {})=>{              // async function to post data to the server
    console.log(projectData);
    const res = await fetch(url, {                                   // waiting to get the url
        method: 'POST',                                              
...
const getData = async (baseUrl, zipCode, apiKey)=>{                             //async function to get data
...
const updateUi = async()=>{                                          // async function for updating the ui
    const res = await fetch('/all');                                 // calling the server using fetch
    try{
        const allData = await res.json();
        document.getElementById('content').innerHTML = allData.user_response;
        document.getElementById('temp').innerHTML = 'Temperaure: ' + allData.temperature + ' F';          // adding data to the page 
